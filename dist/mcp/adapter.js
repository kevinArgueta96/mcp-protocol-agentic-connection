...
export class McpAgentBridge {
    ...
    registerMetaTools() {
        this.server.registerTool("list_agents", {
            description: "List all agents connected to agent-bridge. Shows each agent's project path, type, " +
            ...
            try {
                ...
                return {
                    content: [{ type: "text", text: "Registry not reachable at localhost:4999." }],
                    isError: true,
                };
            }
        });
        ...
        this.server.registerTool("ask_agent", {
            ...
            inputSchema: {
                ...
                input: z.preprocess((v) => (typeof v === "string" ? JSON.parse(v) : v), z.record(z.unknown()).optional()).describe("Direct skill input as JSON (overrides message parsing)"),
            },
            ...
            try {
                ...
                if (artifact?.parts[0]?.type === "data") {
                    return { content: [{ type: "text", text: JSON.stringify(artifact.parts[0].data, null, 2) }] };
                }
            ...
            for (const skill of agent.card.skills) {
                ...
                this.server.registerTool(toolName, {
                    ...
                    try {
                        ...
                        return { content: [{ type: "text", text: JSON.stringify(task.status) }] };
                    }
                    catch (err) {
                        return {
                            content: [{ type: "text", text: `Error: ${err instanceof Error ? err.message : String(err)}` }],
        ...
        switch (skillId) {
            ...
            case "prompt-execute":
                return {
                    ...
                    variables: z.preprocess((v) => (typeof v === "string" ? JSON.parse(v) : v), z.record(z.string()).optional()).describe("Variables to inject"),
    ...
    registerResources(agents) {
        ...
        if (agents.length > 0) {
            ...
            this.server.registerResource("agent-card", template, { description: "A2A Agent Card for a connected agent", mimeType: "application/json" }, async (uri, { agentId }) => {
