...
export class McpAgentBridge {
  ...
  private registerMetaTools(): void {
    ...
    this.server.registerTool(
      ...
      {
        description:
          "List all agents currently connected to agent-bridge. " +
      ...
      async ({ skill, project, healthyOnly = true }) => {
        try {
          ...
          return {
            content: [{ type: "text" as const, text: "Registry not reachable at localhost:4999. Start it with: agent-bridge registry start" }],
            isError: true,
          };
        }
      }
      ...
      async ({ agentId, message, skillId, input }) => {
        try {
          ...
          if (artifact?.parts[0]?.type === "data") {
            return {
              content: [{ type: "text" as const, text: JSON.stringify(artifact.parts[0].data, null, 2) }],
            };
          }
    ...
    this.server.registerTool(
      ...
      {
        description:
          "List files and directory structure of a remote agent's project. " +
        ...
        this.server.registerTool(
          ...
          async (input) => {
            try {
              ...
              return { content: [{ type: "text" as const, text: JSON.stringify(task.status, null, 2) }] };
            } catch (err) {
              return {
                content: [{ type: "text" as const, text: `Error on ${agent.name}: ${err instanceof Error ? err.message : String(err)}` }],
                isError: true,
              };
            }
          }
