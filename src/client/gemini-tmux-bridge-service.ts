...
export interface GeminiTmuxBridgeOptions {
  registryUrl?: string;
  projectPath: string;
  clientId?: string;
  tmuxPane?: string;
...
export class GeminiTmuxBridgeService {
  ...
  async runOnce(): Promise<PendingConversation[]> {
    const client = await this.resolveGeminiClient();
    if (!client) {
      this.log("No Gemini client session resolved");
      return [];
    }
    ...
    this.log(`Resolved Gemini client ${client.agentId} with tmux pane ${pane}`);
  ...
  private async resolveTmuxPane(): Promise<string | undefined> {
    if (this.tmuxPane) {
      ...
      if (isInteractiveGeminiPane(pane)) return this.tmuxPane;
    }

    const session = readCurrentGeminiSession(this.options.projectPath);
    if (session?.tmuxPane) {
      const pane = await getTmuxPaneInfo(session.tmuxPane);
      if (isInteractiveGeminiPane(pane)) return session.tmuxPane;
    }

    const discovered = await discoverGeminiPaneForProject(this.options.projectPath);
    return discovered?.paneId;
  }

  private async resolveGeminiClient() {
    ...
    const sessionFile = readCurrentGeminiSession(this.options.projectPath);
    ...
    return clients.find((entry) =>
      ...
      entry.clientInfo.clientName.toLowerCase().includes("gemini")
    );
  }
  ...
  private async injectConversationPrompt(pane: string, conversation: PendingConversation): Promise<void> {
    ...
    if (!isInteractiveGeminiPane(paneInfo)) {
      this.log(`Skipping injection for ${conversation.conversationId}; pane ${pane} is not running Gemini`);
      return;
    }
    ...
    // Give the Gemini TUI a short window to settle after the paste before
  ...
  private log(message: string): void {
    ...
    console.error(`[Gemini Tmux Sidecar] ${message}`);
  }
}
