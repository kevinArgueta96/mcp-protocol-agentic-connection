...
export class CodexTmuxBridgeService {
  ...
  constructor(private readonly options: CodexTmuxBridgeOptions) {
    ...
    this.statePath = join(options.projectPath, ".open-agent-bridge", "codex-tmux-sidecar-state.json");
  ...
  private async injectConversationPrompt(pane: string, conversation: PendingConversation): Promise<void> {
    ...
    const bufferName = `open-agent-bridge-${process.pid}-${Date.now()}`;
    ...
    await this.channelTransport.postChannelAck({
      ...
      actorId: this.clientId ?? "codex-tmux-sidecar",
      actorType: "client",
      detail: "Codex tmux sidecar injected prompt into the active pane",
    });
  }
