...
function buildInboxReminder(input: {
  ...
  return `New channel message from ${source}.${conversation}${task} Hermes handles channels as inbox-first state. Use channel_inbox to inspect it and reply.${preview}`;
}

/**
 * Behaviour profile for the Hermes Agent CLI/TUI (NousResearch `hermes`).
 * Hermes is an MCP client (`hermes mcp add ... --command open-agent-bridge
 * --args mcp start`), so the channels pattern is delivered as MCP
 * notifications and the agent discovers/answers messages via the
 * open-agent-bridge tools — hence `inbox-first`, matching the
 * Antigravity/Codex inner-client semantics.
 */
export class HermesClientProfile implements ClientBehaviorProfile {
  readonly id = "hermes";
